...
class ProxyThread
  ...
  public ProxyThread(Proxy proxy, Socket s) {
    this.proxy = proxy;
    pSocket = s;
  }

  public ProxyThread(Proxy proxy, Socket s, String server, int port) {
    this.proxy = proxy;
    pSocket = s;
    fwdServer = server;
    fwdPort = port;
  ...
  /**
   * Returns the unique url, inserts the default port 80 if necessary.
   * @param request String
   * @return String
   */
  public String getUrl(String request) {
    final String PORT_80 = ":80";
    final String HTTP = "http://";
    int i = request.indexOf(HTTP);
    if(i != -1) {
      int j = request.indexOf(' ', i);
      String url = request.substring(i + HTTP.length(), j);
      // insert the default port 80
      int k = url.indexOf(':'), l = url.indexOf('/');
      if(k == -1 || l == -1 || k > l) {
        if(l == -1)
          url = url + PORT_80;
        else
          url = url.substring(0, l) + PORT_80 + url.substring(l);
      }
      return url;
    }
    return null;
  ...
  public void run() {
    try {
      ...
      String url = getUrl(new String(request));
      if(proxy.contentProvider != null) {
        response = proxy.contentProvider.getContent(url);
      }

      // content available, stream directly
      if(response != null) {
        debugOut.println("<< " + url + " " + responseLength);
        ...
        if (server != null) {
          ...
          BufferedInputStream serverIn = new BufferedInputStream(server.
              getInputStream());
          BufferedOutputStream serverOut = new BufferedOutputStream(server.
              getOutputStream());
          ...
          //  if(proxy.contentProvider != null) {
          //    debugOut.println("cache >> " + url);
          //    proxy.contentProvider.addContent(url, response);
          //  }
          ...
          responseLength = streamHTTPData(serverIn, clientOut, byteOut, true);
          //debugOut.println("done: " + System.currentTimeMillis());
          ...
          if(proxy.contentProvider != null) {
            ...
            debugOut.println(">> " + url + " " + bytes.length);
            proxy.contentProvider.addContent(url, bytes);
            //debugOut.print(new String(bytes));
          }
        }
  ...
  private byte[] getHTTPData(InputStream in, StringBuffer host,
    ...
    streamHTTPData(in, bs, null, host, waitForDisconnect);
    return bs.toByteArray();
  }

  private int streamHTTPData(InputStream in, OutputStream out, ByteArrayOutputStream byteOut,
                             boolean waitForDisconnect) {
    StringBuffer foo = new StringBuffer("");
    return streamHTTPData(in, out, byteOut, foo, waitForDisconnect);
  }
  ...
  private int streamHTTPData(InputStream in, OutputStream out, ByteArrayOutputStream byteOut,
    ...
    try {
      ...
      if(byteOut != null)
         byteOut.write(header.toString().getBytes(), 0, header.length());
      ...
      if ( (contentLength > 0) || (waitForDisconnect)) {
        try {
          ...
          while ( ( (byteCount < contentLength) || (waitForDisconnect))
            ...
            //debugOut.println("out: " + System.currentTimeMillis());
            out.write(buf, 0, bytesIn);
            out.flush();
            if(byteOut != null)
              byteOut.write(buf, 0, bytesIn);
            byteCount += bytesIn;
          }
        }
        catch (Exception e) {
